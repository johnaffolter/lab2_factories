...
ingest_webhook_email = BashOperator(
    task_id='ingest_webhook_email',
    bash_command='airbyte sync --connection-id {{ params.connection_id }} --workspace-id {{ params.workspace_id }}',
    params={'source_type': 'webhook_email', 'connection_config': {'source': {'name': 'Gmail', 'connector_type': 'source-gmail', 'configuration': {'credentials': {'auth_type': 'oauth2', 'client_id': '{{ var.value.gmail_client_id }}', 'client_secret': '{{ var.value.gmail_client_secret }}', 'refresh_token': '{{ var.value.gmail_refresh_token }}'}, 'start_date': '2024-01-01T00:00:00Z', 'query': 'is:unread', 'batch_size': 100}}, 'destination': {'name': 'PostgreSQL', 'connector_type': 'destination-postgres', 'configuration': {'host': '{{ var.value.postgres_host }}', 'port': 5432, 'database': 'email_analytics', 'username': '{{ var.value.postgres_user }}', 'password': '{{ var.value.postgres_password }}', 'schema': 'raw_emails'}}}, 'sync_mode': 'incremental'},
    dag=dag
)

ingest_streaming_source = BashOperator(
    task_id='ingest_streaming_source',
    bash_command='airbyte sync --connection-id {{ params.connection_id }} --workspace-id {{ params.workspace_id }}',
    params={'source_type': 'streaming_source', 'connection_config': {'source': {'name': 'Gmail', 'connector_type': 'source-gmail', 'configuration': {'credentials': {'auth_type': 'oauth2', 'client_id': '{{ var.value.gmail_client_id }}', 'client_secret': '{{ var.value.gmail_client_secret }}', 'refresh_token': '{{ var.value.gmail_refresh_token }}'}, 'start_date': '2024-01-01T00:00:00Z', 'query': 'is:unread', 'batch_size': 100}}, 'destination': {'name': 'PostgreSQL', 'connector_type': 'destination-postgres', 'configuration': {'host': '{{ var.value.postgres_host }}', 'port': 5432, 'database': 'email_analytics', 'username': '{{ var.value.postgres_user }}', 'password': '{{ var.value.postgres_password }}', 'schema': 'raw_emails'}}}, 'sync_mode': 'incremental'},
    dag=dag
)
...
# Task Dependencies
ingest_webhook_email >> ingest_streaming_source >> validate_ingested_data
...
analyze_email_classification >> analyze_sentiment_analysis >> analyze_grammar_check >> enrich_analysis_results
